...
The general workflow of this code is to 1) update all $b$, starting from $b_{t+h-1,t+h-1}$, ending at $b_{t+h-1,h}$; 2) update all $y^t_{t+h}$ using eqn. 5; 3) update all $MSE^{t}_{t+h}$ using eqn. 6. Note that calculations for $h=1$ needs to be done first before calculating for other $h$ in parallel.
